...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    if (user && !user.isSubscriptionVerified) {
      otp = generateOTP();
      Object.assign(user, {
        password, name, phone, pendingSubscription: subscriptionType,
        selectedCompanies: ['Indeed', 'LinkedIn', 'Glassdoor'], selectedTechnology: 'JavaScript',
      ...
      otp = generateOTP();
      user = new User({
        email, password, name, phone, subscription: 'NONE', pendingSubscription: subscriptionType,
        selectedCompanies: ['Indeed', 'LinkedIn', 'Glassdoor'], selectedTechnology: 'JavaScript',
        otp, otpExpires: Date.now() + 10 * 60 * 1000
      });
      await user.save();
    ...
    res.status(201).json({ message: 'Check OTP at zvertex.247@gmail.com or call +1(918) 924-5130', userId: user._id });
  } catch (error) {
...
router.patch('/user', async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'Preferences updated', user: {
      email: user.email, selectedTechnology: user.selectedTechnology, selectedCompanies: user.selectedCompanies
    }});
  } catch (error) {
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'Check zvertex.247@gmail.com or call +1(918) 924-5130 for reset link' });
  } catch (error) {
