...
requiredEnvVars.forEach(varName => {
  if (!process.env[varName]) {
    console.error(`[${new Date().toISOString()}] Error: Missing environment variable ${varName}`);
    process.exit(1);
  }
});
...
app.use((req, res, next) => {
  ...
  console.log(`[${new Date().toISOString()}] Handling ${req.method} ${req.url} from origin: ${origin}`);
  res.setHeader('Access-Control-Allow-Origin', origin);
  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PATCH, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization');
  res.setHeader('Access-Control-Allow-Credentials', 'true');
  if (req.method === 'OPTIONS') {
    console.log(`[${new Date().toISOString()}] Responding to preflight OPTIONS with 204`);
...
console.log(`[${new Date().toISOString()}] Models registered:`, mongoose.modelNames());
...
app.get('/', (req, res) => {
  ...
  res.status(200).json({ message: 'ZvertexAI Backend API' });
});
