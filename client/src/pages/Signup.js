...
function Signup() {
  const [formData, setFormData] = useState({ email: '', password: '', name: '', phone: '', subscriptionType: 'STUDENT' });
      ...
      <Box sx={{ p: isMobile ? 2 : 4, background: '#fff', borderRadius: 2, boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
        ...
        {!userId ? (
          <form onSubmit={handleSubmit}>
            <TextField fullWidth label="Name" name="name" value={formData.name} onChange={handleChange} margin="normal" required variant="outlined" size={isMobile ? 'small' : 'medium'} />
            ...
            <TextField fullWidth label="Phone" name="phone" value={formData.phone} onChange={handleChange} margin="normal" required variant="outlined" size={isMobile ? 'small' : 'medium'} />
            <TextField fullWidth label="Subscription Type" name="subscriptionType" value={formData.subscriptionType} onChange={handleChange} margin="normal" select SelectProps={{ native: true }} variant="outlined" size={isMobile ? 'small' : 'medium'}>
          ...
          <form onSubmit={handleOtpSubmit}>
            <TextField fullWidth label="Enter OTP" value={otp} onChange={(e) => setOtp(e.target.value)} margin="normal" required variant="outlined" size={isMobile ? 'small' : 'medium'} />
            <Button type="submit" variant="contained" color="primary" fullWidth disabled={loading} sx={{ mt: 3, py: isMobile ? 1 : 1.5, fontSize: isMobile ? '0.9rem' : '1rem' }}>
